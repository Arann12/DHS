...
@section('content')
    ...
    <section class="relative h-screen flex items-center justify-center text-center overflow-hidden">
        ...
        @if(($heroVideoType === 'youtube' || $heroVideoType === 'vimeo') && $heroVideoUrl)
            <iframe class="absolute inset-0 w-full h-full object-cover"
                src="{{ $heroVideoUrl }}{{ str_contains($heroVideoUrl, '?') ? '&autoplay=1&mute=1' : '?autoplay=1&mute=1' }}"
        ...
        <div class="grid md:grid-cols-2 gap-16 md:gap-24 items-center">
            <div data-reveal="fade-right">
                ...
                <div class="space-y-6 text-dhs-darknavy leading-relaxed">
                    <p><span data-id="{{ $aboutP1 }}" data-en="{{ $aboutP1 }}">{{ $aboutP1 }}</span></p>
    ...
    <section class="py-24 bg-dhs-lightblue px-6 md:px-16">
        <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 md:gap-24">
            <div data-reveal="fade-right">
                ...
                <div>
                    <span class="label-text text-primary mb-6 block" data-id="{{ $visionMisiLabel }}"
                        data-en="MISSION">{{ $visionMisiLabel }}</span>
                    <ul class="space-y-4">
                        @foreach($visionMisiItems as $misi)
                            <li class="flex items-start">
                                <span class="material-icons text-primary mr-4 mt-1">check_circle_outline</span>
                                <span class="text-muted-light leading-relaxed" data-id="{{ $misi }}">{{ $misi }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="grid grid-cols-2 gap-6 content-center">
                ...
                @foreach($visionCoreValues as $idx => $val)
                    <div class="bg-background-light p-6 flex flex-col justify-between h-56 transition-transform hover:-translate-y-1 duration-300 shadow-sm card-hover"
                        data-reveal="zoom-in" data-delay="{{ ($idx + 1) * 100 }}">
                        <span class="material-icons text-primary text-3xl">{{ $val['icon'] ?? 'verified' }}</span>
                        <div>
                            <h4 class="font-bold text-sm mb-1 text-text-light" data-id="{{ $val['title'] ?? '' }}">
                                {{ $val['title'] ?? '' }}</h4>
                            <p class="text-[0.7rem] text-muted-light leading-relaxed" data-id="{{ $val['desc'] ?? '' }}">
                                {{ $val['desc'] ?? '' }}</p>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="px-6 md:px-16 max-w-7xl mx-auto">
            <div class="flex flex-col md:flex-row justify-between items-end mb-16" data-reveal="fade-up">
                ...
                <a class="label-text border-b border-primary pb-1 mt-6 md:mt-0 hover:text-primary transition-colors text-primary flex items-center"
                    href="/akademi">
                    <span data-id="LIHAT SEMUA PROGRAM" data-en="VIEW ALL PROGRAMS">LIHAT SEMUA PROGRAM</span> <span
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($academyCards as $idx => $card)
                    <div class="border border-black/10 group cursor-pointer bg-white shadow-sm transition-shadow duration-300 hover:shadow-md card-hover"
                        ...
                        <div class="p-8">
                            <h3 class="text-2xl font-serif mb-4 text-text-light"><span
                                    data-id="{{ $card['title'] ?? '' }}">{{ $card['title'] ?? '' }}</span></h3>
                            <p class="text-muted-light mb-8 text-sm leading-relaxed">
                                <span data-id="{{ $card['desc'] ?? '' }}">{{ $card['desc'] ?? '' }}</span>
                            </p>
                            <a class="label-text text-xs border-b border-text-light/30 pb-1 hover:text-primary transition-colors"
                                href="{{ $card['link'] ?? '/akademi' }}"><span data-id="DETAIL PROGRAM"
                                    data-en="PROGRAM DETAILS">DETAIL PROGRAM</span></a>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @if(isset($facilitiesItems[0]))
                <div class="md:col-span-2 relative h-[500px] md:h-[600px] group overflow-hidden" data-reveal="fade-right">
                    ...
                    <div class="absolute bottom-6 left-6 bg-black/60 backdrop-blur-sm px-4 py-2 text-white text-xs uppercase tracking-wider"
                        data-id="{{ $facilitiesItems[0]['label'] ?? '' }}">{{ $facilitiesItems[0]['label'] ?? '' }}</div>
                </div>
            @endif
                ...
                @foreach(array_slice($facilitiesItems, 1, 2) as $fac)
                    <div class="relative h-1/2 group overflow-hidden">
                        <img alt="{{ $fac['label'] ?? 'Fasilitas' }}"
                            ...
                            src="{{ $fac['image'] ?? '' }}">
                        <div class="absolute bottom-6 left-6 bg-black/60 backdrop-blur-sm px-4 py-2 text-white text-xs uppercase tracking-wider"
                            data-id="{{ $fac['label'] ?? '' }}">{{ $fac['label'] ?? '' }}</div>
                    </div>
                @endforeach
            </div>
        </div>
                ...
                @if($mainArticle)
                    <div class="group cursor-pointer" data-reveal="fade-right">
                        ...
                        <div class="flex items-center space-x-3 mb-4">
                            <span class="label-text text-primary text-[0.6rem]"
                                data-id="{{ strtoupper($mainArticle->category) }}"
                                data-en="{{ strtoupper($mainArticle->category) }}">{{ strtoupper($mainArticle->category) }}</span>
                            <span class="w-1 h-1 rounded-full bg-muted-light/30"></span>
                            <span class="label-text text-muted-light text-[0.6rem]"
                                data-id="{{ $mainArticle->created_at->format('d M Y') }}"
                                data-en="{{ $mainArticle->created_at->format('M d, Y') }}">{{ $mainArticle->created_at->format('d M Y') }}</span>
                        </div>
                        <h3
                        ...
                        <p class="text-muted-light mb-6 leading-relaxed text-sm">
                            <span data-id="{{ $mainArticle->excerpt ?? '' }}"
                    ...
                    @foreach($featuredNews->take(3) as $article)
                        @if($article->id !== ($mainArticle->id ?? null))
                            <a href="/berita/{{ $article->slug }}"
                                class="flex gap-6 group cursor-pointer border-b border-black/5 pb-8 no-underline">
                                ...
                                <div>
                                    <span class="label-text text-primary text-[0.6rem] mb-2 block"
                                        data-id="{{ strtoupper($article->category) }}"
                                    ...
                                    <span class="label-text text-muted-light text-[0.6rem]"
                                        data-id="{{ $article->created_at->format('d M Y') }}"
    ...
    @php
        ...
        $renderCard = function ($item) {
            ...
            $font = is_array($item) ? ($item['font'] ?? 'font-sans text-sm font-bold tracking-wider') : 'font-sans text-sm font-bold tracking-wider';
            ...
            <div class="text-text-light" data-reveal="fade-right">
                ...
                <div class="space-y-8">
                    <div>
                        ...
                        <p class="text-base font-semibold">{{ $contactDenpasarAddr }}</p>
                        <p class="text-sm text-muted-light mt-1">WA: {{ $contactDenpasarWa }}</p>
                        <p class="text-sm text-muted-light">Email: {{ $contactDenpasarEmail }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-sm text-muted-light mt-1">Telp: {{ $contactKlungkungPhone }} | WA:
                            {{ $contactKlungkungWa }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-sm font-medium">Linktree: <a href="{{ $contactLinktreeUrl }}" target="_blank"
                                class="underline text-primary hover:text-dhs-darknavy">{{ parse_url($contactLinktreeUrl, PHP_URL_HOST) ?: 'Linktree' }}</a>
                        </p>

                    </div>
                    {{-- Quick WA button --}}
                    <div class="pt-2">
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $contactDenpasarWa) }}" target="_blank"
                ...
                <form class="space-y-6" @submit.prevent="submitToWhatsApp()">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input x-model="formData.nama" class="contact-field" placeholder="Nama lengkap Anda" type="text"
                                required>
                        </div>
                        <div>
                            <label class="contact-label">EMAIL</label>
                            <input x-model="formData.email" class="contact-field" placeholder="email@example.com"
                                type="email" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="contact-select-wrap">
                            ...
                            <span class="contact-chevron"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                    ...
                    <div>
                        ...
                        <div class="contact-select-wrap"
                            :class="!formData.kategori ? 'opacity-50 pointer-events-none' : ''">
                            <select x-model="formData.program" class="contact-select" :disabled="!formData.kategori"
                                required>
                                <option value="">— Pilih Program —</option>
                                <template x-for="prog in filteredPrograms" :key="prog.id">
                                    <option :value="prog.id"
                                        x-text="(prog.title || prog.program_name || prog) + (prog.duration ? ' (' + prog.duration + ')' : '')">
                                    </option>
                                </template>
                            </select>
                            <span class="contact-chevron"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                    ...
                    <div>
                        ...
                        <textarea x-model="formData.pesan" class="contact-field resize-none"
                            placeholder="Tuliskan pertanyaan atau informasi yang ingin Anda ketahui..." rows="4"
                            required></textarea>
                    </div>

                    <button type="submit"
                        class="w-full py-4 text-white text-sm font-bold tracking-widest uppercase transition-all duration-200 rounded-xl flex items-center justify-center gap-3 hover:shadow-lg active:scale-[0.98] mt-2"
                        style="background:#25D366;">
                        <svg class="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    ...
                    <p class="text-center text-xs text-muted-light">Pesan akan langsung terkirim ke WhatsApp resmi Denpasar
                        Hotel School.</p>
                </form>
            </div>
        </div>
    ...
    <section class="w-full">
        <iframe src="{{ $contactGoogleMapsEmbedUrl }}" width="100%" height="480" style="border:0; display:block;"
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"
            title="Denpasar Hotel School Location">
        </iframe>
