...
<div class="form-group" x-data="imageInput('{{ $inputId }}', '{{ $currentUrl }}')">
    <label for="{{ $inputId }}">
        {{ $inputLabel }}
        @if($isRequired) <span style="color:#E10001;">*</span> @endif
    </label>
    ...
    <div style="display:flex;gap:8px;margin-bottom:12px;border-bottom:2px solid #e0e0e0;">
        <button 
            type="button" 
            @click="mode = 'upload'" 
            :style="mode === 'upload' ? 'border-bottom:3px solid #0E06B4;color:#0E06B4;font-weight:600;' : 'color:#888;'"
        ...
        <button 
            type="button" 
            @click="mode = 'url'" 
            :style="mode === 'url' ? 'border-bottom:3px solid #0E06B4;color:#0E06B4;font-weight:600;' : 'color:#888;'"
            style="background:none;border:none;padding:10px 16px;cursor:pointer;font-size:14px;transition:all 0.2s;margin-bottom:-2px;"
        >
    ...
    <div x-show="imageUrl" x-transition style="margin-top:12px;">
        ...
        <div style="position:relative;display:inline-block;">
            ...
            <button 
                type="button" 
                @click="clearImage()"
                style="position:absolute;top:8px;right:8px;background:#E10001;color:#fff;border:none;border-radius:50%;width:28px;height:28px;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.3);"
                title="Hapus gambar"
            >
