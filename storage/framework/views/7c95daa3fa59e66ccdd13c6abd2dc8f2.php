<?php $__env->startSection('title', 'Berita & Artikel — Denpasar Hotel School'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <section class="max-w-[1280px] mx-auto px-5 md:px-16 pb-12">
        <div class="flex flex-wrap gap-3">
            <a href="/berita"
                class="<?php echo e(!request('kategori') ? 'bg-primary text-white border-primary' : 'bg-white border-black/10 text-text-light hover:border-text-light'); ?> text-[0.7rem] uppercase tracking-[0.15em] font-semibold px-5 py-2 transition-colors border">
                Semua</a>
            <?php $__currentLoopData = ['Prestasi' => 'Prestasi Alumni', 'Partnership' => 'Partnership', 'Kampus' => 'Kampus', 'Akademik' => 'Akademik', 'Kegiatan' => 'Kegiatan', 'Alumni' => 'Alumni', 'Umum' => 'Umum']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $catKey => $catLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="/berita?kategori=<?php echo e($catKey); ?>"
                    class="<?php echo e(request('kategori') === $catKey ? 'bg-primary text-white border-primary' : 'bg-white border-black/10 text-text-light hover:border-text-light'); ?> text-[0.7rem] uppercase tracking-[0.15em] font-semibold px-5 py-2 transition-colors border">
                    <?php echo e($catLabel); ?></a>
    ...
    <section class="max-w-[1280px] mx-auto px-5 md:px-16 pb-16">
        <a href="/berita/<?php echo e($featured->slug); ?>" class="block group">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-0 bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div class="md:col-span-7 h-72 md:h-[420px] overflow-hidden">
                    <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        ...
                        src="<?php echo e($featured->thumbnail_url ?: 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?q=80&w=1600'); ?>">
                </div>
                <div class="md:col-span-5 p-8 md:p-12 flex flex-col justify-center">
                    <div class="flex items-center gap-3 mb-4">
                        <span class="text-[0.65rem] uppercase tracking-[0.15em] font-bold text-primary bg-primary/10 px-3 py-1 rounded"><?php echo e($featured->category); ?></span>
                        <span class="text-xs text-muted-light"><?php echo e($featured->published_at?->format('d M Y') ?? ''); ?></span>
                    </div>
                    ...
                    <p class="text-sm text-muted-light leading-relaxed line-clamp-4">
                        <?php echo strip_tags(Str::limit($featured->excerpt ?? $featured->content, 250)); ?>
    ...
    <section class="max-w-[1280px] mx-auto px-5 md:px-16 pt-4 pb-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm flex flex-col group cursor-pointer hover:shadow-md transition-shadow border border-black/5">
                <a href="/berita/<?php echo e($article->slug); ?>" class="flex flex-col flex-1">
                <div class="aspect-[16/10] overflow-hidden bg-gray-100">
                    <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        alt="<?php echo e($article->title); ?>"
                        src="<?php echo e($article->thumbnail_url ?: 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?q=80&w=800'); ?>">
                </div>
                <div class="p-5 flex flex-col gap-3 flex-1">
                    <div class="flex items-center gap-3">
                        <span class="text-[0.65rem] uppercase tracking-[0.15em] font-bold text-primary bg-primary/10 px-2.5 py-0.5 rounded"><?php echo e($article->category); ?></span>
                        <span class="text-xs text-muted-light"><?php echo e($article->published_at?->format('d M Y') ?? ''); ?></span>
                    ...
                    <p class="text-sm text-muted-light line-clamp-2 leading-relaxed"><?php echo strip_tags(Str::limit($article->excerpt ?? $article->content, 120)); ?></p>
                </div>
                </a>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <div class="mt-12"><?php echo e($articles->withQueryString()->links()); ?></div>
        <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\DHS\resources\views/berita.blade.php ENDPATH**/ ?>